...
// Note: This type is thread-safe for concurrent use via atomic counter increment.
...
func (p *PseudoRandom) Float64() float64 {
	...
	x := uint64(p.Seed) + c*6364136223846793005
	x ^= x >> 12
	x ^= x << 25
	x ^= x >> 27
...
// Uses modular arithmetic for O(1) performance regardless of angle magnitude.
